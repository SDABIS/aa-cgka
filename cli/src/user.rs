...
use ds_lib::{ClientKeyPackages, DsGroupInfo, GroupMessage};
use openmls::prelude::*;
use openmls_traits::OpenMlsProvider;
use tls_codec::TlsByteVecU8;
use openmls::extensions::ExtensionType::SsiVcRequirements;
use openmls::helpers::message_out_to_group_info;
use openmls::prelude::ProposalType::{AddReqs, RemoveReqs, UpdateReqs};
...
impl User {
    ...
    pub fn new_from_credential(
        ...
        let out = Self {
            ...
            group_list: HashSet::new(),
            contacts: HashMap::new(),
            identity: RefCell::new(Identity::new_from_credential(
                CIPHERSUITE,
                &crypto,
                signature_keys,
                credential_with_key,
    ...
    pub fn get_group_info(&self, name: String) -> Result<DsGroupInfo, String> {
        self.backend.group_info(name)
    }
    ...
    pub fn create_kp(&self) {
        ...
        match self.backend.publish_key_packages(self, &ckp) {
            Ok(()) => (),
            Err(e) => log::info!("Error sending new key package: {e:?}"),
        };
    }

    pub fn add_requirement(&mut self, req: Value, group_name: String) -> Result<(), String> {
        let groups = self.groups.borrow();
        ...
        let (out_messages, _, new_group_info) = group
            .mls_group
            .borrow_mut()
            .add_new_reqs(
                &self.crypto,
        ...
        let msg = GroupMessage::new(out_messages.into(), &self.recipients(group));
        log::debug!(" >>> send: {:?}", msg);
        match self.backend.send_msg(&msg) {
            Ok(()) => (),
            Err(e) => log::info!("Error sending group message: {e:?}"),
        }

        // Second, process the commit on our end.
        group
            .mls_group
            .borrow_mut()
        ...
        let updated_group_info = DsGroupInfo::new(
            new_group_info.unwrap().into(),
            group.mls_group.borrow().export_ratchet_tree()
        );
        self.backend
            .publish_group_info(group_name, updated_group_info)
            .expect("Error sending GroupInfo message");
    ...
    pub fn update(
        ...
        > {
            ...
            log::info!("{} RECEIVED MESSAGE", self.username);
            ...
            let mut file = OpenOptions::new()
                ...
                .write(true)
                .append(true)
                .open("validation_times_bbsvc_add.txt")
                .unwrap();

            if let Err(e) = writeln!(file, "{}", format!("{members}\t{elapsed}")) {
...
//                    process_protocol_message(message.into()).expect("Error processing message");
    ...
    pub fn create_group(&mut self, name: String) {
        ...
        let group_info_out =
            &group.mls_group.borrow().export_group_info(self.crypto.crypto(), &self.identity.borrow().signer, false
        ).expect("Error Parsing GroupInfo")
                .into_verifiable_group_info().unwrap();

        let updated_group_info = DsGroupInfo::new(
            group_info_out.clone(),
            group.mls_group.borrow().export_ratchet_tree()
        );
        self.backend
            .publish_group_info(name.clone(), updated_group_info)
            .expect("Error sending GroupInfo message");
    ...
    pub fn modify_req(&mut self, index: u32, new_req: &Value, group_name: String) -> Result<(), String> {
        // Get the group ID
        ...
        // Remove operation on the mls group
        let (remove_message, _welcome, new_group_info) = group
            .mls_group
            .borrow_mut()
            .update_reqs(&self.crypto, &self.identity.borrow().signer, new_req, index)
        ...
        // Second, process the proposal on our end.
        group
            .mls_group
            .borrow_mut()
            .merge_pending_commit(&self.crypto)
        ...
        self.backend
            .publish_group_info(group_name.clone(), updated_group_info)
            .expect("Error sending GroupInfo message");
    ...
    /// Invite user with the given name to the group.
    pub fn external_join(&mut self, group_name: String, group_info: DsGroupInfo) -> Result<(), String> {
        self.update_clients();
        ...
        let (mut new_mls_group, out_messages, new_group_info) = MlsGroup::join_by_external_commit(
            &self.crypto,
            &self.identity.borrow().signer,
            Some(group_info.ratchet_tree.into()),
        ...
        log::trace!("Sending commit");
        let group_recipients = self.recipients(&new_group);
        log::info!("{} joined. Recipients: {:?}", self.username, group_recipients.iter()
            .map(|a| from_utf8(a).unwrap()).collect::<Vec<&str>>());
        ...
        // Finally, send GroupInfo.
        let updated_group_info;
        log::trace!("Sending new group info");
        if let Some(gi) = new_group_info {
            updated_group_info = DsGroupInfo::new(
                gi.into(),
                new_group.mls_group.borrow().export_ratchet_tree()
            );
        }
        else {
            updated_group_info = DsGroupInfo::new(
                message_out_to_group_info(
                    &new_group.mls_group.borrow().export_group_info(
                        self.crypto.crypto(), &self.identity.borrow().signer, false)
        ...
        match groups.insert(group_name, new_group) {
