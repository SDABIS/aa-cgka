...
impl Backend {
    ...
    pub fn group_info(&self, group_name: String) -> Result<DsGroupInfo, String> {
        let mut url = self.ds_url.clone();
        ...
        let response = get(&url)?;
        match DsGroupInfo::tls_deserialize(&mut response.as_slice()) {
            Ok(gi) => Ok(gi),
            Err(e) => Err(format!("Error decoding server response: {e:?}")),
        }
    }

    pub fn publish_group_info(&self, group_name: String, group_info: DsGroupInfo) -> Result<(), String> {
        let mut url = self.ds_url.clone();
        let path = "/groups/group_info/".to_string()
            + &base64::encode_config(group_name, base64::URL_SAFE);
        url.set_path(&path);
        //log::info!("{url}");
        let response = post(&url,&group_info)?;
        Ok(())
    }
    ...
    pub fn recv_msgs(&self, user: &User) -> Result<Vec<MlsMessageIn>, String> {
        ...
        let response = get(&url)?;
        match TlsVecU32::<MlsMessageIn>::tls_deserialize(&mut response.as_slice()) {
            Ok(r) => Ok(r.into()),
