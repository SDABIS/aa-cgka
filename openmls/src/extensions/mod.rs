...
pub use ssi_vc_extension::{SsiVcRequirement, SsiVcRequirementsExtension};
...
pub enum ExtensionType {
    ...
    /// Group Info Extension for establishing the VC requirements for external joins.
    SsiVcRequirements,
...
impl From<u16> for ExtensionType {
    fn from(a: u16) -> Self {
        match a {
            ...
            6 => ExtensionType::SsiVcRequirements,
...
impl From<ExtensionType> for u16 {
    fn from(value: ExtensionType) -> Self {
        match value {
            ...
            ExtensionType::SsiVcRequirements => 6,
...
pub enum Extension {
    ...
    /// A [`SsiVcRequirementsExtension`]
    SsiVcRequirements(SsiVcRequirementsExtension),
...
impl Extensions {
    ...
    pub fn ssi_vc_requirements(&self) -> Option<&SsiVcRequirementsExtension> {
        self.find_by_type(ExtensionType::SsiVcRequirements)
            .and_then(|e| match e {
                Extension::SsiVcRequirements(e) => Some(e),
                _ => None,
            })
    }

    pub fn ssi_vc_requirements_mut(&mut self) -> Option<&mut SsiVcRequirementsExtension> {
        self.find_by_type_mut(ExtensionType::SsiVcRequirements)
            .and_then(|e| match e {
                Extension::SsiVcRequirements(e) => Some(e),
                _ => None,
            })
...
impl Extension {
    ...
    pub const fn extension_type(&self) -> ExtensionType {
        match self {
            ...
            Extension::SsiVcRequirements(_) => ExtensionType::SsiVcRequirements,
    ...
    fn add_try_from() {
        ...
        for (test, should_work) in tests.into_iter() {
            // Test `add`.
            {
                ...
                log::info!("{:?}, {:?}", test.clone(), should_work);
