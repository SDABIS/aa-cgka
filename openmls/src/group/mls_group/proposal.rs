...
impl MlsGroup {
    ...
    pub fn propose<KeyStore: OpenMlsKeyStore>(
        ...
        match propose {
            ...
            Propose::AddReqs(new_req) => {
                self.propose_add_reqs(provider, signer, &new_req)
            }
            Propose::UpdateReqs(index, new_req) => {
                self.propose_update_reqs(provider, signer, index, &new_req)
                .map_err(|e| e.into())
            }
            Propose::RemoveReqs(index) => {
                self.propose_remove_reqs(provider, signer, index)
            }
        }
    }
    ...
    pub fn propose_update_reqs(
        ...
        let proposal_update_reqs = self
            .group
            .create_update_reqs_proposal(self.framing_parameters(), index, new_req, signer)
            .map_err(|_| ProposeRemoveMemberError::UnknownMember)?;

        let proposal = QueuedProposal::from_authenticated_content_by_ref(
            self.ciphersuite(),
            provider.crypto(),
            proposal_update_reqs.clone(),
        )?;
        let proposal_ref = proposal.proposal_reference();
        self.proposal_store.add(proposal);

        let mls_message = self.content_to_mls_message(proposal_update_reqs, provider)?;
