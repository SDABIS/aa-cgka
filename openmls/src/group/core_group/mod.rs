...
impl CoreGroupBuilder {
    ...
    pub(crate) fn with_ssi_vc_requirements(
        mut self,
        ssi_vc_requirements: SsiVcRequirementsExtension,
    ) -> Self {
        self.public_group_builder = self
            .public_group_builder
            .with_ssi_vc_requirements(ssi_vc_requirements);
        self
    }
...
impl CoreGroup {
    ...
    pub(crate) fn create_add_reqs_proposal(
        &self,
        framing_parameters: FramingParameters,
        new_req: &Value,
        signer: &impl Signer,
        ...
        let serialised_req = serde_json::to_vec(new_req)
            .map_err(|_| LibraryError::custom("Error serialising new Requirement"))?
            .into();
        let add_reqs_proposal = AddReqsProposal {new_req: serialised_req};
        let proposal = Proposal::AddReqs(add_reqs_proposal);
        AuthenticatedContent::member_proposal(
            framing_parameters,
            self.own_leaf_index(),
            proposal,
    ...
    pub(crate) fn create_update_reqs_proposal(
        &self,
        framing_parameters: FramingParameters,
        index: u32,
        updated_req: &Value,
        ...
        let serialised_req = serde_json::to_vec(updated_req)
            .map_err(|_| LibraryError::custom("Error serialising new Requirement"))?
            .into();
        let update_reqs_proposal = UpdateReqsProposal {index, new_req: serialised_req};
        let proposal = Proposal::UpdateReqs(update_reqs_proposal);
        AuthenticatedContent::member_proposal(
            framing_parameters,
            self.own_leaf_index(),
            proposal,
    ...
    pub(crate) fn create_remove_reqs_proposal(
        &self,
        framing_parameters: FramingParameters,
        index: u32,
        signer: &impl Signer,
    ) -> Result<AuthenticatedContent, LibraryError> {
        let remove_reqs_proposal = RemoveReqsProposal {index};
        let proposal = Proposal::RemoveReqs(remove_reqs_proposal);
        AuthenticatedContent::member_proposal(
            framing_parameters,
            self.own_leaf_index(),
            proposal,
