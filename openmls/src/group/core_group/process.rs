...
impl CoreGroup {
    ...
    pub(crate) fn process_unverified_message(
        ...
        //  - ValSem010
        //  - ValSem246 (as part of ValSem010)
        let credential_with_key = unverified_message.credential_with_key();
        ...
        match content.sender() {
            Sender::Member(_) | Sender::NewMemberCommit | Sender::NewMemberProposal => {
                ...
                // If the received commit adds a new member to the group
                if let Sender::NewMemberCommit = content.sender() {
                    //log::info!("\tValidating {}'s Credential", std::str::from_utf8(credential_with_key.credential.identity().into()).expect("AAAA"));
                    credential_with_key.validate().map_err(|_| ValidationError::InvalidSignature)?;
                    //log::info!("\t\tCredential's signature is valid");
                    //log::info!("\t> Checking Requirements");
                    if let Some(e) = self.public_group.group_context().extensions().ssi_vc_requirements() {
                        credential_with_key.check_requirements(e)
                            .map_err(|_| ValidationError::UnauthorizedExternalSender)?;
                        //log::info!("\t\tCredential Matches Requirement\n");
                    }
                }
