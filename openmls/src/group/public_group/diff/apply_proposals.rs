...
use crate::prelude::SsiVcRequirement;
...
impl<'a> PublicGroupDiff<'a> {
    pub(crate) fn apply_proposals(
        ...
        for queued_proposal in proposal_queue.filtered_by_type(ProposalType::UpdateReqs) {
            if let Some(requirements) = self.group_context.extensions_mut().ssi_vc_requirements_mut() {
                if let Proposal::UpdateReqs(update_proposal) = queued_proposal.proposal().to_owned() {
                    requirements[update_proposal.index as usize] = SsiVcRequirement { requirement: update_proposal.new_req };
                }
            }
            else {
                return Err(LibraryError::custom("Proposal to modify requirements, but requirements do not exist"))
            }
        }

        for queued_proposal in proposal_queue.filtered_by_type(ProposalType::RemoveReqs) {
            if let Some(requirements) = self.group_context.extensions_mut().ssi_vc_requirements_mut() {
                if let Proposal::RemoveReqs(ref remove_proposal) = queued_proposal.proposal() {
                    requirements.remove(remove_proposal.index as usize);
                }
            }
            else {
                return Err(LibraryError::custom("Proposal to modify requirements, but requirements do not exist"))
        ...
        for queued_proposal in proposal_queue.filtered_by_type(ProposalType::AddReqs) {
            if let Some(requirements) = self.group_context.extensions_mut().ssi_vc_requirements_mut() {
                if let Proposal::AddReqs(add_proposal) = queued_proposal.proposal().to_owned() {
                    requirements.push(SsiVcRequirement { requirement: add_proposal.new_req });
                }
            }
            else {
                return Err(LibraryError::custom("Proposal to modify requirements, but requirements do not exist"))
