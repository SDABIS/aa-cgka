...
impl PublicGroup {
    ...
    pub(crate) fn validate_add_proposals(
        ...
        for add_proposal in add_proposals {
            let credential_with_key = add_proposal.add_proposal().key_package.leaf_node().credential_with_key();
            if let Some(e) = self.group_context().extensions().ssi_vc_requirements() {
                credential_with_key.check_requirements(e)
                    .map_err(|_| ProposalValidationError::InvalidAddProposalCiphersuiteOrVersion)?;
            }
            credential_with_key.validate()
                .map_err(|_| ProposalValidationError::InvalidAddProposalCiphersuiteOrVersion)?;
            ...
            if proposal.proposal_or_ref_type() == ProposalOrRefType::Proposal {
                if let Proposal::Remove(remove_proposal) = proposal.proposal() {
                    ...
                    if let Some(new_leaf) = path_leaf_node {
                        ...
                        {
                            /*log::info!("{} != {}", std::str::from_utf8(removed_leaf.credential().identity()).unwrap(),
                                std::str::from_utf8(new_leaf.credential().identity()).unwrap());*/
