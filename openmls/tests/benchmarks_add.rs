...
// ... and the crypto backend to use.
    ...
    while group_members < 100 {
        ...
        for i in 0..ITERS_FOR_CRED {
            ...
            let mut group = add_member_through_proposal(
                &group_config_with_requirement,
                &mut alice_group,
                &alice_signer.clone(),
                daniel_key_package,
        ...
        let now = Instant::now();
        for i in 0..ITERS_FOR_CRED {
            let (ellen_credential_with_key, ellen_signer) = generate_basic_credential_with_key(
                "ellen".into(),
                ciphersuite.signature_algorithm(),
                backend,
            );
        ...
        let mut file = OpenOptions::new()
            ...
            .write(true)
            .append(true)
            .open("generation_times_unified_add.txt")
            .unwrap();
