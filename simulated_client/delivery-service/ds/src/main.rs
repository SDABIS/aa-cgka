...
pub struct DsData {
    ...
    group_info: Mutex<HashMap<Vec<u8>, (VerifiableGroupInfo, bool)>>,
}
...
async fn publish_key_packages(
    ...
    let client = match clients.get_mut(&id) {
        Some(client) => client,
        None => {
            log::error!("Client not found: {:?}", from_utf8(id.as_slice()).unwrap());
    ...
    key_packages
        ...
        .for_each(|value| {
            //log::debug!("Adding KP: {:?} to {}", value, from_utf8(id.as_slice()).unwrap());
            client.key_packages.0.push(value)
        });
...
async fn consume_key_package(path: web::Path<String>, data: web::Data<DsData>) -> impl Responder {
    ...
    log::debug!("Consuming key package for {}", std::str::from_utf8(id.as_slice()).unwrap());
...
async fn get_group_info(path: web::Path<String>, params: web::Query<UsernameQuery>, data: web::Data<DsData>) -> impl Responder {
    ...
    let group_info = match groups.get(&id) {
        Some((group_info, available)) => {
            match available {
               true => group_info.clone(),
    ...
    if data.group_info_mutex {
        groups.insert(id, (group_info.clone(), false));
    }
...
async fn group_exists(path: web::Path<String>, data: web::Data<DsData>) -> impl Responder {
    ...
    if let Some((group_info, _)) = groups.get(&id) {
        actix_web::HttpResponse::Ok().body(unwrap_data!(group_info.tls_serialize_detached()))
    }
    else {
...
async fn post_group_info(
    ...
    groups.insert(id, (group_info, true));
