...
impl<'a> CommitBuilder<'a, Initial> {
    ...
    pub fn propose_add_reqs(mut self, added: impl IntoIterator<Item = Value>) -> Self {
        self.stage.own_proposals.extend(
            added
                .into_iter()
                .map(|added| Proposal::AddReqs(AddReqsProposal { new_req: serde_json::to_vec(&added).unwrap().into() })),
        );
        self
    }

    pub fn propose_update_reqs(mut self, index: u32, new_req: Value) -> Self {
        self.stage.own_proposals.push(
            Proposal::UpdateReqs(UpdateReqsProposal {
                index,
                new_req: serde_json::to_vec(&new_req).unwrap().into(),
            }),
        );
        self
    }

    pub fn propose_removal_reqs(mut self, removed: impl IntoIterator<Item = u32>) -> Self {
        self.stage.own_proposals.extend(
            removed
                .into_iter()
                .map(|removed| Proposal::RemoveReqs(RemoveReqsProposal { index: removed })),
        );
        self
    }
