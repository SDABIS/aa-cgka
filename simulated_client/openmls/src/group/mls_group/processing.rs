        ...
        match content.sender() {
            Sender::Member(_) | Sender::NewMemberCommit | Sender::NewMemberProposal => {
                ...
                if let Sender::NewMemberCommit = content.sender() {
                    ...
                    let option_requirement = self.public_group.group_context().extensions().ssi_vc_requirements();

                    credential_with_key.validate(option_requirement.cloned()).map_err(|_| ValidationError::InvalidSignature)?;
                }
