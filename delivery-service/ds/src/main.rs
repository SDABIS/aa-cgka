...
use tls_codec::{Deserialize, Serialize, TlsSliceU16, TlsSliceU32, TlsVecU32};
...
pub struct DsData {
    ...
    group_info: Mutex<HashMap<Vec<u8>, DsGroupInfo>>,
}
...
async fn get_group_info(path: web::Path<String>, data: web::Data<DsData>) -> impl Responder {
    ...
    log::debug!("Getting group info of {:?}", std::str::from_utf8(id.as_slice()).unwrap());
...
async fn post_group_info(
    ...
    log::debug!("Add group info for {:?}", std::str::from_utf8(id.as_slice()).unwrap());

    let group_info = match DsGroupInfo::tls_deserialize(&mut &bytes[..]) {
        Ok(GI) => GI,
        Err(_) => {
            log::error!(
